    ...
    return {
        ...
        init: function (opts) {
            ...
            _socket
                ...
                .on('options:highlights', function (data) {
                    _highlights = data;
                })
        ...
        log: function (data) {
            ...
            if (_highlights) {
                data = String(data);
                if (_highlights.words) {
                    for (var wordCheck in _highlights.words) {
                        data = data.replace(wordCheck, '<span style="' + _highlights.words[wordCheck] + '">' + wordCheck + '</span>');
                    }
                }
                if (_highlights.lines) {
                    for (var lineCheck in _highlights.lines) {
                        if (data.indexOf(lineCheck) !== -1) {
                            p.setAttribute('style', _highlights.lines[lineCheck]);
                        }
                    }
                }
            }
