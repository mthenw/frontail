...
window.App = (function app(window, document) {
  ...
  var _filterElement = function(elem) {
    ...
    var pattern2 = new RegExp(_invFilterValue, 'i');
    var element = elem;
    if (pattern.test(element.textContent) &&
        (_invFilterValue === '' || !pattern2.test(element.textContent))) {
  ...
  return {
    ...
    init: function init(opts) {
      ...
      _invFilterInput = opts.invFilterInput;
      _filterInput.focus();
      _topbar = opts.topbar;
      _body = opts.body;
      ...
      // Filter input bind
      _invFilterInput.addEventListener('keyup', function(e) {
        // ESC
        if (e.keyCode === 27) {
          this.value = '';
          _invFilterValue = '';
        } else {
          _invFilterValue = this.value;
        }
        _filterLogs();
      });
